...
load(paste0(path, "fits/Fmat/Fmat_aic3.RData"))
aic <- aic[order(aic$AIC), ]
min(aic$AIC) # m_d_2nb9b
...
cov_datasets <- holdout(model = mod_ds, training_cutoff = 0.7)
...
cov_cv <- sdmTMB( 
  ...
  formula = count ~ fgear + fyear + s(middepth_scaled, m = 1) + substrate,
  mesh = mesh_cov,
  family = nbinom2(),
...
saveRDS(file = paste0(path, "/fits/Fmat/", "cv_fits.rds"), crossvalfits)

sink(file = paste0(path, "/fits/Fmat/", "crossvalidation_results.txt"))
...
saveRDS(preds, file = paste0(path, "/fits/Fmat/","prediction_mapping_data.RData"))
