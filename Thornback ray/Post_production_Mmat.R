...
### Check AIC ------
mods_mmat <- readRDS(paste0(path, "fits/Mmat/Mmat_model_object4.rds"))
names(mods_mmat)

all_aic <- map_dfr(names(mods_mmat), function(name) {
  model <- mods_mmat[[name]]
  tibble(
    Model = name,
    AIC = model$aic,
    #cAIC = model$caic
...
sink(file = paste0(path, "/fits/Mmat/","aic.txt"))
all_aic
sink(file = NULL)

### Read data ------
sanity(mods_mmat$m_d_2nb3[[1]])
sanity(mods_mmat$m_d_2nb9[[1]])

mod_spattemp <- mods_mmat$m_d_2nb3[[1]]
mod_dss <- mods_mmat$m_d_2nb9[[1]] # best

sink(file = paste0(path, "/fits/Mmat/","base_model.txt"))
mod_spattemp
...
sink(file = paste0(path, "/fits/Mmat/","cov_model.txt"))
mod_dss
...
names(modelIndices) <- c("mod_spattemp", 
                         #"mod_depth", 
                         "mod_dss")
...
### Visualise indices
index_mmat <- ggplot(all_indices, aes(year, est, colour = model), size=2) + geom_line() +
...
ggsave(filename = paste0(path, "/fits/Mmat/","nbinom_indices.jpg"), plot = index_mmat)
saveRDS(index_mmat, file = paste0(plot_path, "Mmat_index_ribbon.rds"))

index_mmat2 <- ggplot(all_indices, aes(year, est, colour = model, linetype = model), size=2) + 
  geom_line() +
  ...
  ggtitle("Abundance index by model approach: Male mature Thornback ray")
ggsave(filename = paste0(path, "/fits/Mmat/","nbinom_indices2.jpg"), plot = index_mmat2)
saveRDS(index_mmat2, file = paste0(plot_path, "/mmat_index.rds"))

all_indices$CI_width <- with(all_indices, upr - lwr)
head(all_indices)

index_mmat_error <- ggplot(all_indices, aes(year, CI_width, colour = model, linetype = model), size=2) + 
  geom_line() +
  scale_color_manual(values = c("grey", "black")) +
  labs(fill = "Model", colour = "Model", linetype = "Model") +
  ...
  theme(axis.text = element_text(size = 13), 
        ...
        legend.background = element_rect(linewidth = 1, colour = "black"), 
        plot.margin = margin(5, 20, 5, 5)) +
  ggtitle("Index error by model approach: Male mature Thornback ray")
ggsave(filename = paste0(path, "/fits/mmat/","nbinom_indices_error.jpg"), plot = index_mmat_error)
saveRDS(index_mmat_error, file = paste0(plot_path, "mmat_index_CI.rds"))

index_mmat_se <- ggplot(all_indices, aes(year, log(se), colour = model, linetype = model), size=2) + 
  geom_line() +
  scale_color_manual(values = c("grey", "black")) +
  labs(fill = "Model", colour = "Model", linetype = "Model") +
  xlab('Year') + ylab('log SE of index') +
  ...
  theme(axis.text = element_text(size = 13), 
        ...
        plot.margin = margin(5, 20, 5, 5)) +
  ggtitle("Index error by model approach: Male mature Thornback ray")
ggsave(filename = paste0(path, "/fits/mmat/","nbinom_indices_se.jpg"), plot = index_mmat_se)
saveRDS(index_mmat_se, file = paste0(plot_path, "/mmat_index_se.rds"))

save(all_indices, file = paste0(path, "/fits/Mmat/", "allindices.RData"))

### Make plot of error
mods_mmat$m_d_2nb3$rf$model <- "base"
t <- mods_mmat$m_d_2nb3$rf
mods_mmat$m_d_2nb9$rf$model <- "base+substrate+depth+bottomT"
t2 <- mods_mmat$m_d_2nb9$rf
all_rfs <- rbind(t, t2)
...
rfs_plot2D <- ggplot(all_rfs_sub, aes(term, estimate, fill = model)) +
  ...
  labs(x = "Random effect", y = "Estimate", fill = "Model", 
       title = "Random effects: Male mature Thornback ray") +
  theme_bw() +
  theme(axis.text = element_text(size = 13), 
        axis.title = element_text(size = 14, face = "bold"),
        legend.title = element_text(size = 13, face = "bold"),
        ...
        legend.background = element_rect(linewidth = 1, colour = "black"), 
        plot.margin = margin(5, 20, 5, 5))
ggsave(filename = paste0(path, "/fits/Mmat/","rf_2D.jpg"), plot = rfs_plot2D)
saveRDS(rfs_plot2D, file = paste0(plot_path, "/Mmat_rf.rds"))
...
saveRDS(mar_eff_list_base, paste0(path, "/fits/Mmat/", "allmarginaleffects_base.rds"))

mar_effs_base <- y_cov / g_cov 
ggsave(filename = paste0(path, "/fits/Mmat/","mareffplot_base.jpg"), plot = mar_effs_base)
mar_effs2_base <- y_cov2 / g_cov2
ggsave(filename = paste0(path, "/fits/Mmat/","mareffplot2_base.jpg"), plot = mar_effs2_base)
...
d_cov <- visreg(mod_dss, xvar = "middepth_scaled", data = mod_dss$data, gg = TRUE)
...
saveRDS(mar_eff_list, paste0(path, "/fits/Mmat/", "allmarginaleffects.rds"))


mar_effs <- (y_cov + g_cov) / d_cov / (substr_cov + sbt_cov)
ggsave(filename = paste0(path, "/fits/Mmat/","mareffplot.jpg"), plot = mar_effs)
mar_effs2 <- (y_cov2 + g_cov2) / d_cov2 / (substr_cov2 + sbt_cov2)
ggsave(filename = paste0(path, "/fits/Mmat/","mareffplot2.jpg"), plot = mar_effs2)


### Partial deviance explained (PDE) -----
mod_depth <- mods_mmat$m_d_2nb4[[1]]
mod_substr <- mods_mmat$m_d_2nb6[[1]]
mod_sbt <- mods_mmat$m_d_2nb5[[1]]
...
sink(file = paste0(path, "/fits/Mmat/", "pde_report.txt"))
...
write.csv(all_terms_base, 
          file = paste0(path, "/fits/Mmat/", "base_factor_level_pvalues.csv"))
...
write.csv(all_terms_cov, 
          file = paste0(path, "/fits/Mmat/", "cov_factor_level_pvalues.csv"))
...
png(paste0(path, "/fits/Mmat/", "randomised_quantile_base_bestfit.png"))
par(mfrow = c(1,2))
...
png(paste0(path, "/fits/Mmat/", "dharma_covbestfit_test.png"))
sink(file = paste0(path, "/fits/Mmat/", "dharma_standardtests_covmod.txt"))
DHARMa::testResiduals(s_mod_cov)
sink(file = NULL)
dev.off()
png(paste0(path, "/fits/Mmat/", "dharma_base_test.png"))
sink(file = paste0(path, "/fits/Mmat/", "dharma_standardtests_basemod.txt"))
DHARMa::testResiduals(s_mod_base)
sink(file = NULL)
dev.off()

# Res vs preds
png(paste0(path, "/fits/Mmat/", "dharma_mods_vs_preds.png"))
par(mfrow = c(1,2))
...
png(paste0(path, "/fits/Mmat/", "dharma_bestfit_res_vs_covs.png"))
par(mfrow = c(3,1))
...
# Zeroinflation
png(paste0(path, "/fits/Mmat/", "zeroinflation_mods.png"))
par(mfrow = c(1,2))
...
title(main = list("Base model (left), Covariate model (right)", cex = 1.4), 
      line = -3.3, outer = TRUE)
dev.off()
sink(file = paste0(path, "/fits/Mmat/", "dharma_zeroinf_basemod.txt"))
DHARMa::testZeroInflation(s_mod_base)
sink(file = NULL)
sink(file = paste0(path, "/fits/Mmat/", "dharma_zeroinf_covmod.txt"))
DHARMa::testZeroInflation(s_mod_cov)
sink(file = NULL)

# Dispersion
png(paste0(path, "/fits/Mmat/", "dispersion_mods.png"))
par(mfrow = c(1,2))
...
title(main = list("Base model (left), Covariate model (right)", cex = 1.4), 
      line = -3.3, outer = TRUE)
dev.off()
sink(file = paste0(path, "/fits/Mmat/", "dharma_dispersion_basemod.txt"))
DHARMa::testDispersion(s_mod_base)
sink(file = NULL)
sink(file = paste0(path, "/fits/Mmat/", "dharma_dispersion_covmod.txt"))
DHARMa::testDispersion(s_mod_cov)
sink(file = NULL)
...
ggsave(filename = paste0(path, "/fits/Mmat/","spatial_res_basemod.png"), plot = spatial_basemod)
spatial_covmod <- ggplot(mod_dss$data, aes(X, Y, col = resids)) + scale_colour_gradient2() +
  geom_point() + facet_wrap(~year) + coord_fixed()
ggsave(filename = paste0(path, "/fits/Mmat/","spatial_res_covmod.png"), plot = spatial_covmod)
...
sink(file = paste0(path, "/fits/Mmat/","global_morans_results_base.txt"))
...
morans_spattemp
sink(file = NULL)
sink(file = paste0(path, "/fits/Mmat/","global_morans_results_cov.txt"))
cat("Sample size in each year \n")
n_cov
cat("\n")
cat("Moran's I in each year \n")
...
ggsave(filename = paste0(path, "/fits/Mmat/", "globm_plots_base.jpg"), 
       buffered_base_grid)
...
buffered_cov_grid <- cowplot::ggdraw() +
  cowplot::draw_plot(globmorans_cov_grid, 
                     x = 0.05, y = 0.05, width = 0.9, height = 0.9)
ggsave(filename = paste0(path, "/fits/Mmat/", "globm_plots_cov.jpg"), 
       buffered_cov_grid)
...
sink(file = paste0(path, "/fits/Mmat/","local_morans_psignificance_base.txt"))
...
sink(file = paste0(path, "/fits/Mmat/","local_morans_psignificance_cov.txt"))
print("Sum of coordinates with p-value significance below alpha level threshold 0.01")
print(locm_cov_sig)
sink(file = NULL)
...
sink(file = paste0(path, "/fits/Mmat/","getis_ord_psignificance_base.txt"))
print("Sum of coordinates with p-value significance below alpha level threshold 0.01")
...
sink(file = paste0(path, "/fits/Mmat/","getis_ord_psignificance_cov.txt"))
print("Sum of coordinates with p-value significance below alpha level threshold 0.01")
...
saveRDS(spat_autocor_res, file = paste0(path, "/fits/Mmat/", "spatialAutocorResults.rds"))
...
save_plot(paste0(path, "/fits/Mmat/","locm_plots_base.png"), 
...
save_plot(paste0(path, "/fits/Mmat/","locm_plots_cov.png"), locm_plots_cov)
...
save_plot(paste0(path, "/fits/Mmat/","getis_ord_plots_base.png"), gord_plots_base)
...
save_plot(paste0(path, "/fits/Mmat/","getis_ord_plots_cov.png"), gord_plots_cov)
...
save_plot(paste0(path, "/fits/Mmat/","getis_ord_hotspot_plots_base.png"), hotspot_plots_base)
...
save_plot(paste0(path, "/fits/Mmat/","getis_ord_hotspot_plots_cov.png"), 
...
base_density <- plot_map(predictions_spattemp$data, exp(est)) + 
  ...
  theme_map()
ggsave(filename = paste0(path, "/fits/Mmat/", "density_base.jpg"), plot = base_density, dpi = 300)

cov_density <- plot_map(predictions_d_s_substr$data, exp(est)) + 
  ggtitle("Covariate model: Density") +
  scale_fill_viridis_c(option = "G", direction = -1) +
  theme_map()
ggsave(filename = paste0(path, "/fits/Mmat/", "density_cov.jpg"), plot = cov_density, dpi = 300)
...
ggsave(filename = paste0(path, "/fits/Mmat/","density_se_base.jpg"), plot = est_uncertainty_base)
...
est_uncertainty_cov <- plot_map(grid_yrs, exp(est_se_cov)) +
  ...
  theme_map() +
  theme(plot.title = element_text(hjust = 0.5, size = 13, face = "bold"))
ggsave(filename = paste0(path, "/fits/Mmat/","density_se_cov.jpg"), plot = est_uncertainty_cov)
...
base_ff <- plot_map2(predictions_spattemp$data, est_non_rf) + 
  ...
  theme_map() +
  theme(plot.title = element_text(hjust = 0.5, size = 13, face = "bold"))
#ggsave(filename = paste0(path, "/fits/Mmat/","ff_effects_base.jpg"), plot = base_ff)
...
cov_ff <- plot_map2(predictions_d_s_substr$data, est_non_rf) +
  ...
  theme_map() +
  theme(plot.title = element_text(hjust = 0.5, size = 13, face = "bold"))
#ggsave(filename = paste0(path, "/fits/Mmat/","ff_effects_cov.jpg"), plot = cov_ff)
...
ggsave(filename = paste0(path, "/fits/Mmat/","fixed_effects_base_cov.jpg"), plot = ff_plots)
...
base_omega <- plot_map2(predictions_spattemp$data, omega_s) +
  ...
  theme_map() +
  theme(plot.title = element_text(hjust = 0.5, size = 13, face = "bold"))
#ggsave(filename = paste0(path, "/fits/Mmat/","omega_effects_base.png"), plot = base_omega)
...
cov_omega <- plot_map2(predictions_d_s_substr$data, omega_s) +
  ...
  theme_map() +
  theme(plot.title = element_text(hjust = 0.5, size = 13, face = "bold"))
#ggsave(filename = paste0(path, "/fits/Mmat/","omega_effects_cov.png"), plot = cov_omega)
...
omega_uncertainty_base <- plot_map2(grid_yrs, omega_se_base) +
  ...
  theme_map() +
  theme(plot.title = element_text(hjust = 0.5, size = 13, face = "bold"))
#ggsave(filename = paste0(path, "/fits/Mmat/","omega_se_base.png"), plot = omega_uncertainty_base)
...
omega_uncertainty_cov <- plot_map2(grid_yrs, omega_se_cov) +
  ...
  theme_map() +
  theme(plot.title = element_text(hjust = 0.5, size = 13, face = "bold"))
#ggsave(filename = paste0(path, "/fits/Mmat/","omega_se_cov.png"), plot = omega_uncertainty_cov)
...
ggsave(filename = paste0(path, "/fits/Mmat/","all_omega_and_se_base_cov.jpg"), plot = omega)
...
ggsave(filename = paste0(path, "/fits/Mmat/","eps_effects_base.jpg"), plot = base_eps)
...
cov_eps <- plot_map(predictions_d_s_substr$data, epsilon_st) + 
  ...
  theme_map() +
  theme(plot.title = element_text(hjust = 0.5, size = 15, face = "bold"))
ggsave(filename = paste0(path, "/fits/Mmat/","eps_effects_cov.jpg"), plot = cov_eps)
...
eps_uncertainty_base <- plot_map(grid_yrs, eps_se_base) +
  ...
  theme_map() +
  theme(plot.title = element_text(hjust = 0.5, size = 15, face = "bold"))
ggsave(filename = paste0(path, "/fits/Mmat/","eps_se_base.jpg"), plot = eps_uncertainty_base)
...
eps_uncertainty_cov <- plot_map(grid_yrs, eps_se_cov) +
  ...
  theme_map() +
  theme(plot.title = element_text(hjust = 0.5, size = 15, face = "bold"))
ggsave(filename = paste0(path, "/fits/Mmat/","eps_se_cov.jpg"), plot = eps_uncertainty_cov)

# Save all predictions
save(grid_yrs, file = paste0(path, "/fits/Mmat/","uncertainty_mapping_data.RData"))
preds2 <- list("base" = predictions_spattemp, "cov_full" = predictions_d_s_substr)
saveRDS(preds2, file = paste0(path, "/fits/Mmat/","prediction_mapping_data.rds"))
