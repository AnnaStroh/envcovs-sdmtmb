...
#        sqrt(mean((base_cv$data$count - base_cv$data$cv_predicted)^2)), "\n")
#  cat("  MAE between original abundance and CV predictions across entire dataset:",
#        mean(abs(base_cv$data$count - base_cv$data$cv_predicted)), "\n")
...
#  cat(" Model validation: predictive accuracy with fitted model", "\n")
#  cat("  RMSE between original abundance and CV predictions across entire dataset:",
#        sqrt(mean((cov_cv$data$count - cov_cv$data$cv_predicted)^2)), "\n")
#  cat("  MAE between original abundance and CV predictions across entire dataset:", 
#        mean(abs(cov_cv$data$count - cov_cv$data$cv_predicted)), "\n")
...
crossval_report <- function(base_preds, cov_preds) {
  ...
  cat(" Model validation: predictive accuracy with fitted model", "\n")
  cat("  RMSE between original abundance and CV predictions across entire dataset:", 
      sqrt(mean((base_preds$count - base_preds$est)^2, na.rm = TRUE)), "\n")
  cat("  MAE between original abundance and CV predictions across entire dataset:",
      mean(abs(base_preds$count - base_preds$est), na.rm = TRUE), "\n")
  cat("\n")
  cat("BEST FIT COVARIATE MODEL", "\n")
  cat("70:30 Holdout crossvalidation", "\n")
  cat(" Model validation: predictive accuracy with fitted model", "\n")
  cat("  RMSE between original abundance and CV predictions across entire dataset:",
      sqrt(mean((cov_preds$count - cov_preds$est)^2, na.rm = TRUE)), "\n")
  cat("  MAE between original abundance and CV predictions across entire dataset:", 
      mean(abs(cov_preds$count - cov_preds$est), na.rm = TRUE), "\n")
  cat("\n")
}
