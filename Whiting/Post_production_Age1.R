...
load(paste0(path, "fits/1/1_aic.RData"))
aic
min(aic$AIC) # depth model

sink(file = paste0(path, "/fits/1/", "aic.txt"))
aic
sink(file = NULL)

### Read data ------

mods_1 <- readRDS(paste0(path, "fits/1/1_model_object.rds"))
names(mods_1)

sanity(mods_1$m_dlogn_spattemp[[1]])
sanity(mods_1$m_dlogn3[[1]]) 

mod_spattemp <- mods_1$m_dlogn_spattemp[[1]]
mod_depth <- mods_1$m_dlogn3[[1]]

sink(file = paste0(path, "/fits/1/", "base_model.txt"))
mod_spattemp
...
sink(file = paste0(path, "/fits/1/", "cov_model.txt"))
mod_depth
...
# Including depth
predictions_d <- predict(mod_depth, newdata = grid_yrs, return_tmb_object = TRUE)
...
### Visualise indices ------
index_1 <- ggplot(all_indices, aes(year, est, colour = model), size=2) + geom_line() +
  ...
  ggtitle("Abundance index by model approach: Whiting age 1")
ggsave(filename = paste0(path, "/fits/1/","logn_indices.jpg"), plot = index_1)

index_1_2 <- ggplot(all_indices, aes(year, est, colour = model, linetype = model), size=2) + 
...
ggsave(filename = paste0(path, "/fits/1/","indices2.jpg"), plot = index_1_2, 
       width = 2100, height = 2100, units = "px")
saveRDS(index_1_2, file = paste0(plot_path, "/1_index2.rds"))

all_indices$CI_width <- with(all_indices, upr - lwr)

index_1_error <- ggplot(all_indices, aes(year, CI_width, colour = model, linetype = model), size=2) + 
  geom_line(linewidth = 2) +
  ...
  theme(axis.text = element_text(size = 15), 
        ...
        legend.background = element_rect(linewidth = 1, colour = "black"), 
        plot.margin = margin(5, 20, 5, 5))
ggsave(filename = paste0(path, "/fits/1/","1_index_CI.jpg"), plot = index_1_error)
saveRDS(index_1_error, file = paste0(plot_path, "1_index_CI.rds"))

index_1_se <- ggplot(all_indices, aes(year, log(se), colour = model, linetype = model), size=2) + 
  geom_line(linewidth = 2) +
  scale_color_manual(values = c("grey", "black")) +
  labs(fill = "Model", colour = "Model", linetype = "Model") +
  ...
  theme(axis.text = element_text(size = 15), 
        ...
        legend.background = element_rect(linewidth = 1, colour = "black"), 
        plot.margin = margin(5, 20, 5, 5))
ggsave(filename = paste0(path, "/fits/1/","indices_se.jpg"), plot = index_1_se, 
       width = 2100, height = 2100, units = "px")
saveRDS(index_1_se, file = paste0(plot_path, "/1_index_se.rds"))

#save(all_indices, file = paste0(path, "/fits/1/", "allindices.RData"))
...
abundance <- abundance[, -c(1,3)] # also remove VAST-based age 0 index
...
base_index <- data.frame(year = base_sub$year, 
                         age = 1, 
                         index = base_sub$est)
df_base <- rbind(long[c(1:21),], base_index, long[c(22:nrow(long)),])
df_base$cohort <- with(df_base, year - age)
df_base_wide <- reshape::cast(df_base, cohort ~ age, value = "index")

png(filename = paste0(path, "/fits/1/", "intconsistency_base1.png"))
...
cov_index <- data.frame(year = cov_sub$year, 
                        age = 1, 
                        index = cov_sub$est)
df_cov <- rbind(long[c(1:21),], cov_index, long[c(22:nrow(long)),])
df_cov$cohort <- with(df_cov, year - age)
df_cov_wide <- reshape::cast(df_cov, cohort ~ age, value = "index")

png(filename = paste0(path, "/fits/1/", "intconsistency_cov1.png"))
...
saveRDS(consistency_dat, file = paste0(path, "/fits/1/", "consistency_data.rds"))
...
cov_rf_enc <- tidy(mod_depth, model = 1, effects = "ran_pars")
cov_rf_pos <- tidy(mod_depth, model = 2, effects = "ran_pars")
...
save(all_rfs_sub, file = paste0(path, "/fits/1/", "rf_data.RData"))
...
rfs_plot2D <- ggplot(all_rfs_sub, aes(term, estimate, fill = model)) +
  ...
  theme(axis.text = element_text(size = 15), 
        ...
        legend.background = element_rect(linewidth = 1, colour = "black"), 
        plot.margin = margin(5, 20, 5, 5))
ggsave(filename = paste0(path, "/fits/1/","rf_2D.jpg"), plot = rfs_plot2D, 
       width = 2100, height = 2100, units = "px")
saveRDS(rfs_plot2D, file = paste0(plot_path, "/whg_1_rf.rds"))
...
ggsave(filename = paste0(path, "/fits/1/" ,"year_effect_base.jpg"), plot = y_p_base)

# Covariate model
# year
y_cova <- visreg_delta(mod_depth, xvar = "fyear", model = 1, 
                       gg = TRUE)
y_covb <- visreg_delta(mod_depth, xvar = "fyear", model = 2, 
                       gg = TRUE)
y_cov2a <- visreg_delta(mod_depth, xvar = "fyear", 
                        model = 1, scale = "response", gg = TRUE, 
                        partial = FALSE, rug = FALSE)
y_cov2b <- visreg_delta(mod_depth, xvar = "fyear", 
                        model = 2, trans = exp, gg = TRUE, 
                        partial = FALSE, rug = FALSE)
y_cov2adat <- visreg_delta(mod_depth, xvar = "fyear", 
                        model = 1, scale = "response", gg = TRUE, 
                        partial = FALSE, rug = FALSE, plot = FALSE)
y_cov2bdat <- visreg_delta(mod_depth, xvar = "fyear", 
                        model = 2, trans = exp, gg = TRUE, 
                        partial = FALSE, rug = FALSE, plot = FALSE)
y_p <- format_mareff_plots(mareff1 = y_cov2a, mareff2 = y_cov2b)
ggsave(filename = paste0(path, "/fits/1/","year_effect_cov.jpg"), plot = y_p)

# depth
d_cova <- visreg_delta(mod_depth, xvar = "middepth_scaled", model = 1,
                       gg = TRUE)
d_covb <- visreg_delta(mod_depth, xvar = "middepth_scaled", model = 2,
                       gg = TRUE)
d_cov2a <- visreg_delta(mod_depth, xvar = "middepth_scaled", 
                        model = 1, scale = "response", gg = TRUE, 
                        partial = FALSE, rug = FALSE)
d_cov2b <- visreg_delta(mod_depth, xvar = "middepth_scaled", 
                        model = 2, trans = exp, gg = TRUE, 
                        partial = FALSE, rug = FALSE)
d_p <- format_mareff_plots(mareff1 = d_cov2a, mareff2 = d_cov2b)
ggsave(filename = paste0(path, "/fits/1/","depth_effect.jpg"), plot = d_p, 
       width = 2100, height = 2100, units = "px")
...
mar_eff_list <- list("fyear2_base_m1" = y_base2a, "fyear2_base_m2" = y_base2b,
                     ...
                     "depth_2_m1" = d_cov2a, "depth_2_m2" = d_cov2b)
saveRDS(mar_eff_list, paste0(path, "/fits/1/", "allmarginaleffects.rds"))
...
saveRDS(year_eff_list, paste0(path, "/fits/1/", "year_marginaleffects.rds"))
...
sink(file = paste0(path, "/fits/1/", "pde_report.txt"))
deviance_report(full_cov_model = mod_depth, null_model = null_model, 
...
write.csv(all_terms_base, 
          file = paste0(path, "/fits/1/", "base_factor_level_pvalues.csv"))

# cov - year + covariates
# factor effects
SDrepFixed_cov <- as.data.frame(summary(mod_depth$sd_report, 
                                        p.value=TRUE, select = "fixed"))
...
terms_cov <- rbind(tidy(mod_depth, model = 1), 
                   tidy(mod_depth, model = 2))
...
write.csv(all_terms_cov, 
          file = paste0(path, "/fits/1/", "cov_factor_level_pvalues.csv"))
...
cov_datasets <- holdout(model = mod_depth, training_cutoff = 0.7)
...
base_cv <- sdmTMB(
  ...
  mesh = mesh_base,
  family = delta_lognormal(),
  offset = log(base_datasets$training$areakmsqadj),
  spatial = list("on","on"),
  time = "year", 
...
saveRDS(file = paste0(path, "fits/1/", "cv_fits.rds"), crossvalfits)

sink(file = paste0(path, "fits/1/", "crossvalidation_results.txt"))
...
mod_cov_res1 <- residuals(mod_depth, type = "mle-mvn", model = 1)
mod_cov_res2 <- residuals(mod_depth, type = "mle-mvn", model = 2)

png(paste0(path, "fits/1/", "randomised_quantile_base_bestfit.png"))
par(mfrow = c(2,2))
...
s_mod_cov <- make_dharma(model = mod_depth, model_comp = c(1, 2))

# Uniformity, dispersion, outliers
png(paste0(path, "/fits/1/", "dharma_covbestfit_test_m1.png"))
sink(file = paste0(path, "/fits/1/", "dharma_standardtests_covmod_m1.txt"))
DHARMa::testResiduals(s_mod_cov$m1)
sink(file = NULL)
dev.off()
png(paste0(path, "/fits/1/", "dharma_covbestfit_test_m2.png"))
sink(file = paste0(path, "/fits/1/", "dharma_standardtests_covmod_m2.txt"))
DHARMa::testResiduals(s_mod_cov$m2)
sink(file = NULL)
dev.off()
png(paste0(path, "/fits/1/", "dharma_base_test_m1.png"))
sink(file = paste0(path, "/fits/1/", "dharma_standardtests_basemod_m1.txt"))
DHARMa::testResiduals(s_mod_base$m1)
sink(file = NULL)
dev.off()
png(paste0(path, "/fits/1/", "dharma_base_test_m2.png"))
sink(file = paste0(path, "/fits/1/", "dharma_standardtests_basemod_m2.txt"))
DHARMa::testResiduals(s_mod_base$m2)
sink(file = NULL)
dev.off()

# Res vs preds
png(paste0(path, "/fits/1/", "dharma_mods_vs_preds.png"))
par(mfrow = c(2,2))
...
mod_plot_data <- subset(mod_depth$data, biomass > 0)
png(paste0(path, "/fits/1/", "dharma_bestfit_res_vs_covs.png"))
par(mfrow = c(3,2))
DHARMa::plotResiduals(s_mod_cov$m1, mod_depth$data$fyear)
DHARMa::plotResiduals(s_mod_cov$m2, mod_plot_data$fyear)
DHARMa::plotResiduals(s_mod_cov$m1, mod_depth$data$middepth_scaled)
...
png(paste0(path, "/fits/1/", "dharma_base_res_vs_years.png"))
par(mfrow = c(1,2))
...
# Zeroinflation
png(paste0(path, "/fits/1/", "zeroinflation_mods.png"))
par(mfrow = c(2,2))
...
title(main = list("Base model (left), Covariate model (right)", cex = 1.4), 
      line = -3.3, outer = TRUE)
dev.off()
sink(file = paste0(path, "/fits/1/", "dharma_zeroinf_basemod_m1.txt"))
DHARMa::testZeroInflation(s_mod_base$m1)
sink(file = NULL)
sink(file = paste0(path, "/fits/1/", "dharma_zeroinf_basemod_m2.txt"))
DHARMa::testZeroInflation(s_mod_base$m2)
sink(file = NULL)
sink(file = paste0(path, "/fits/1/", "dharma_zeroinf_covmod_m1.txt"))
DHARMa::testZeroInflation(s_mod_cov$m1)
sink(file = NULL)
sink(file = paste0(path, "/fits/1/", "dharma_zeroinf_covmod_m2.txt"))
DHARMa::testZeroInflation(s_mod_cov$m2)
sink(file = NULL)

# Uniformity
png(paste0(path, "/fits/1/", "uniformity_mods.png"))
par(mfrow = c(2,2))
...
title(main = list("Base model (left), Covariate model (right)", cex = 1.4), 
      line = -3.3, outer = TRUE)
dev.off()
sink(file = paste0(path, "/fits/1/", "dharma_uniformity_basemod_m1.txt"))
DHARMa::testUniformity(s_mod_base$m1)
sink(file = NULL)
sink(file = paste0(path, "/fits/1/", "dharma_uniformity_basemod_m2.txt"))
DHARMa::testUniformity(s_mod_base$m2)
sink(file = NULL)
sink(file = paste0(path, "/fits/1/", "dharma_uniformity_covmod.txt"))
DHARMa::testUniformity(s_mod_cov$m1)
sink(file = NULL)
sink(file = paste0(path, "/fits/1/", "dharma_uniformity_covmod.txt"))
DHARMa::testUniformity(s_mod_cov$m2)
...
# Dispersion
png(paste0(path, "/fits/1/", "dispersion_mods.png"))
par(mfrow = c(2,2))
...
title(main = list("Base model (left), Covariate model (right)", cex = 1.4), 
      line = -3.3, outer = TRUE)
dev.off()
sink(file = paste0(path, "/fits/1/", "dharma_dispersion_basemod_m1.txt"))
DHARMa::testDispersion(s_mod_base$m1)
sink(file = NULL)
sink(file = paste0(path, "/fits/1/", "dharma_dispersion_basemod_m2.txt"))
DHARMa::testDispersion(s_mod_base$m2)
sink(file = NULL)
sink(file = paste0(path, "/fits/1/", "dharma_dispersion_covmod.txt"))
DHARMa::testDispersion(s_mod_cov$m1)
sink(file = NULL)
sink(file = paste0(path, "/fits/1/", "dharma_dispersion_covmod.txt"))
DHARMa::testDispersion(s_mod_cov$m2)
...
grouped_basemod1 <- recalculateResiduals(s_mod_base$m1, group = unique(mod_depth$data$fyear))
grouped_basemod2 <- recalculateResiduals(s_mod_base$m2, group = unique(mod_plot_data$fyear))

grouped_covmod1<- recalculateResiduals(s_mod_cov$m1, group = unique(mod_depth$data$fyear))
grouped_covmod2 <- recalculateResiduals(s_mod_cov$m2, group = unique(mod_plot_data$fyear))

png(paste0(path, "/fits/1/", "tempautocorr_basemod_m1.png"))
DHARMa::testTemporalAutocorrelation(grouped_basemod1, time = unique(mod_depth$data$fyear))
title(main = list("Base model", cex = 1.4), 
      line = -1, outer = TRUE)
dev.off()
png(paste0(path, "/fits/1/", "tempautocorr_basemod_m2.png"))
DHARMa::testTemporalAutocorrelation(grouped_basemod2, time = unique(mod_plot_data$fyear))
title(main = list("Base model", cex = 1.4), 
      line = -1, outer = TRUE)
dev.off()
png(paste0(path, "/fits/1/", "tempautocorr_covmod_m1.png"))
DHARMa::testTemporalAutocorrelation(grouped_covmod1, time = unique(mod_depth$data$fyear))
title(main = list("Covariate model", cex = 1.4), 
      line = -1, outer = TRUE)
dev.off()
png(paste0(path, "/fits/1/", "tempautocorr_covmod_m2.png"))
DHARMa::testTemporalAutocorrelation(grouped_covmod2, time = unique(mod_plot_data$fyear))
title(main = list("Covariate model", cex = 1.4), 
      line = -1, outer = TRUE)
dev.off()
sink(file = paste0(path, "/fits/1/", "dharma_dispersion_basemod_m1.txt"))
testTemporalAutocorrelation(grouped_basemod1, time = unique(mod_depth$data$fyear))
sink(file = NULL)
sink(file = paste0(path, "/fits/1/", "dharma_dispersion_basemod_m2.txt"))
testTemporalAutocorrelation(grouped_basemod2, time = unique(mod_plot_data$fyear))
sink(file = NULL)
sink(file = paste0(path, "/fits/1/", "dharma_dispersion_covmod_m1.txt"))
testTemporalAutocorrelation(grouped_covmod1, time = unique(mod_depth$data$fyear))
sink(file = NULL)
sink(file = paste0(path, "/fits/1/", "dharma_dispersion_covmod_m2.txt"))
...
mod_depth$data$resids <- mod_cov_res1
mod_depth$data$resids2 <- mod_cov_res2
...
# cov
ggplot(data=mod_depth$data, aes(x=resids, group=fyear, fill=fyear)) +
  geom_density(adjust=1.5, alpha=.4) # binomial
ggplot(data=mod_depth$data, aes(x=resids2, group=fyear, fill=fyear)) +
  geom_density(adjust=1.5, alpha=.4) # lognormal
...
ggsave(filename = paste0(path, "fits/1/", "spatial_res_basemod_m1.png"), plot = spatial_basemod1)
spatial_basemod2 <- ggplot(mod_spattemp$data, aes(X, Y, col = resids2)) + scale_colour_gradient2() +
  geom_point() + facet_wrap(~year) + coord_fixed()
ggsave(filename = paste0(path, "fits/1/", "spatial_res_basemod_m2.png"), plot = spatial_basemod2)
spatial_covmod1 <- ggplot(mod_depth$data, aes(X, Y, col = resids)) + scale_colour_gradient2() +
  geom_point() + facet_wrap(~year) + coord_fixed()
ggsave(filename = paste0(path, "fits/1/", "spatial_res_covmod_m1.png"), plot = spatial_covmod1)
spatial_covmod2 <- ggplot(mod_depth$data, aes(X, Y, col = resids2)) + scale_colour_gradient2() +
  geom_point() + facet_wrap(~year) + coord_fixed()
ggsave(filename = paste0(path, "fits/1/", "spatial_res_covmod_m2.png"), plot = spatial_covmod2)
...
sink(file = paste0(path, "fits/1/","global_morans_results_base_binom.txt"))
...
morans_spattemp_m1
sink(file = NULL)
sink(file = paste0(path, "fits/1/","global_morans_results_base_logn.txt"))
cat("Sample size in each year \n")
n_spattemp_m2
cat("\n")
cat("Moran's I in each year \n")
...
ggsave(filename = paste0(path, "fits/1/", "globm_plots_base_binom.jpg"), 
...
buffered_spattemp_m2_grid <- cowplot::ggdraw() +
  cowplot::draw_plot(globmorans_spattemp_m2_grid, 
                     x = 0.05, y = 0.05, width = 0.9, height = 0.9)
ggsave(filename = paste0(path, "fits/1/", "globm_plots_base_logn.jpg"), 
       buffered_spattemp_m2_grid)


# covariate
knw_cov_binom <- make_kn_dist_obj_binom(coords = mod_depth$data)
knw_cov_pos <- make_kn_dist_obj_pos(coords = mod_depth$data)
...
sink(file = paste0(path, "fits/1/","global_morans_results_cov_binom.txt"))
cat("Sample size in each year \n")
n_cov_m1
cat("\n")
cat("Moran's I in each year \n")
morans_cov_m1
sink(file = NULL)
sink(file = paste0(path, "fits/1/","global_morans_results_cov_logn.txt"))
cat("Sample size in each year \n")
n_cov_m2
cat("\n")
cat("Moran's I in each year \n")
...
buffered_cov_m1_grid <- cowplot::ggdraw() +
  cowplot::draw_plot(globmorans_cov_m1_grid, 
                     x = 0.05, y = 0.05, width = 0.9, height = 0.9)
ggsave(filename = paste0(path, "fits/1/", "globm_plots_cov_binom.jpg"), 
...
buffered_cov_m2_grid <- cowplot::ggdraw() +
  cowplot::draw_plot(globmorans_cov_m2_grid, 
                     x = 0.05, y = 0.05, width = 0.9, height = 0.9)
ggsave(filename = paste0(path, "fits/1/", "globm_plots_cov_logn.jpg"), 
...
save(locm_spattemp_binom_df, 
     file = paste0(path, "fits/1/","local_morans_psignificance_base_binom.RData"))
locm_spattemp_binom_sig <- purrr::map(locm_spattemp_binom, ~ check_psig_locm(.x))
sink(file = paste0(path, "fits/1/","local_morans_psignificance_base_binom.txt"))
...
save(locm_spattemp_pos_df, 
     file = paste0(path, "fits/1/","local_morans_psignificance_base_pos.RData"))
locm_spattemp_pos_sig <- purrr::map(locm_spattemp_pos, ~ check_psig_locm(.x))
sink(file = paste0(path, "fits/1/","local_morans_psignificance_base_pos.txt"))
print("Sum of coordinates with p-value significance below alpha level threshold 0.01")
...
save(locm_cov_binom_df, 
     file = paste0(path, "fits/1/","local_morans_psignificance_cov_binom.RData"))
locm_cov_binom_sig <- purrr::map(locm_cov_binom, ~ check_psig_locm(.x))
sink(file = paste0(path, "fits/1/","local_morans_psignificance_cov_binom.txt"))
print("Sum of coordinates with p-value significance below alpha level threshold 0.01")
...
save(locm_cov_pos_df, 
     file = paste0(path, "fits/1/","local_morans_psignificance_cov_pos.RData"))
locm_cov_pos_sig <- purrr::map(locm_cov_pos, ~ check_psig_locm(.x))
sink(file = paste0(path, "fits/1/","local_morans_psignificance_cov_pos.txt"))
print("Sum of coordinates with p-value significance below alpha level threshold 0.01")
...
save(getisord_spattemp_binom_df, 
     file = paste0(path, "fits/1/","getis_ord_psignificance_spattemp_binom.RData"))
getisord_spattemp_binom_sig <- purrr::map(getisord_spattemp_binom, ~ check_psig_locg(.x))
sink(file = paste0(path, "fits/1/","getis_ord_psignificance_base_binom.txt"))
print("Sum of coordinates with p-value significance below alpha level threshold 0.01")
...
save(getisord_spattemp_pos_df, 
     file = paste0(path, "fits/1/","getis_ord_psignificance_spattemp_pos.RData"))
getisord_spattemp_pos_sig <- purrr::map(getisord_spattemp_pos, ~ check_psig_locg(.x))
sink(file = paste0(path, "fits/1/","getis_ord_psignificance_base_pos.txt"))
print("Sum of coordinates with p-value significance below alpha level threshold 0.01")
...
save(getisord_cov_binom_df, 
     file = paste0(path, "fits/1/","getis_ord_psignificance_cov_binom.RData"))
getisord_cov_binom_sig <- purrr::map(getisord_cov_binom, ~ check_psig_locg(.x))
sink(file = paste0(path, "fits/1/","getis_ord_psignificance_cov_binom.txt"))
print("Sum of coordinates with p-value significance below alpha level threshold 0.01")
...
save(getisord_cov_pos_df, 
     file = paste0(path, "fits/1/","getis_ord_psignificance_cov_pos.RData"))
getisord_cov_pos_sig <- purrr::map(getisord_cov_pos, ~ check_psig_locg(.x))
sink(file = paste0(path, "fits/1/","getis_ord_psignificance_cov_pos.txt"))
print("Sum of coordinates with p-value significance below alpha level threshold 0.01")
...
saveRDS(spat_autocor_res, file = paste0(path, "fits/1/", "spatialAutocorResults.rds"))
...
cowplot::save_plot(paste0(path, "fits/1/","locm_plots_base_binom.jpg"), 
...
cowplot::save_plot(paste0(path, "fits/1/","locm_plots_base_logn.jpg"), 
...
cowplot::save_plot(paste0(path, "fits/1/", "locm_plots_cov_binom.jpg"), locm_plots_cov_binom)
...
cowplot::save_plot(paste0(path, "fits/1/", "locm_plots_cov_logn.jpg"), locm_plots_cov_logn)
...
cowplot::save_plot(paste0(path, "fits/1/", "getis_ord_plots_base_binom.jpg"), gord_plots_base_binom)
...
cowplot::save_plot(paste0(path, "fits/1/", "getis_ord_plots_base_logn.jpg"), gord_plots_base_logn)
...
cowplot::save_plot(paste0(path, "fits/1/", "getis_ord_plots_cov_binom.jpg"), gord_plots_cov_binom)
...
cowplot::save_plot(paste0(path, "fits/1/", "getis_ord_plots_cov_logn.jpg"), gord_plots_cov_logn)
...
cowplot::save_plot(paste0(path, "fits/1/","getis_ord_hotspot_plots_base_binom.jpg"), hotspot_plots_base_binom)
...
cowplot::save_plot(paste0(path, "fits/1/","getis_ord_hotspot_plots_base_logn.jpg"), 
...
cowplot::save_plot(paste0(path, "fits/1/","getis_ord_hotspot_plots_cov_binom.jpg"), 
...
cowplot::save_plot(paste0(path, "fits/1/","getis_ord_hotspot_plots_cov_logn.jpg"), 
...
saveRDS(predictions_spattemp, file = paste0(path, "fits/1/", "fullobj_base_model_predictions.rds"))
write.csv(predictions_spattemp$data, file = paste0(path, "fits/1/", "base_model_predictions.csv"), row.names = FALSE)

predictions_d$data$density <- plogis(predictions_d$data$est1) * exp(predictions_d$data$est2)
saveRDS(predictions_d, file = paste0(path, "fits/1/", "fullobj_cov_model_predictions.rds"))
write.csv(predictions_d$data, file = paste0(path, "fits/1/", "cov_model_predictions.csv"), row.names = FALSE)
...
ggsave(filename = paste0(path, "fits/1/", "density_base.jpg"), plot = base_density, dpi = 300)

cov_density <- plot_map(predictions_d$data, density) + 
  ggtitle("Covariate model: Density") +
  scale_fill_viridis_c(option = "G", direction = -1) +
  theme_map() + 
  theme(legend.position = c(.85,.05))
ggsave(filename = paste0(path, "fits/1/", "density_cov.jpg"), plot = cov_density, dpi = 300)
...
ggsave(filename = paste0(path, "fits/1/","est_se_base1.jpg"), plot = est_uncertainty_base1)
...
est_uncertainty_base2 <- plot_map(grid_yrs, base_est2_se) +
  ...
  theme(plot.title = element_text(hjust = 0.5, size = 13, face = "bold"),
        legend.position = c(.85,.05))
ggsave(filename = paste0(path, "fits/1/","est_se_base2.jpg"), plot = est_uncertainty_base2)


# covariate model
predictions_cov_est1 <- predict(mod_depth, 
                                newdata = grid_yrs,
                                model = 1, nsim = 500, 
                                sims_var = "est")
...
predictions_cov_est2 <- predict(mod_depth, 
                                newdata = grid_yrs, 
                                model = 2, nsim = 500, 
                                sims_var = "est")
...
est_uncertainty_cov1 <- plot_map(grid_yrs, cov_est1_se) +
  ...
  theme(plot.title = element_text(hjust = 0.5, size = 13, face = "bold"),
        legend.position = c(.85,.05))
ggsave(filename = paste0(path, "fits/1/","est_se_cov1.jpg"), plot = est_uncertainty_cov1)
...
est_uncertainty_cov2 <- plot_map(grid_yrs, cov_est2_se) +
  ...
  theme(plot.title = element_text(hjust = 0.5, size = 13, face = "bold"),
        legend.position = c(.85,.05))
ggsave(filename = paste0(path, "fits/1/","est_se_cov2.jpg"), plot = est_uncertainty_cov2)
...
base_est1 <- plot_map(predictions_spattemp$data, plogis(est1)) + 
  ...
  theme_map() + 
  theme(legend.position = c(.85,.05))
ggsave(filename = paste0(path, "fits/1/","enc_prob_base.jpg"), plot = base_est1, dpi = 300)
...
base_est2 <- plot_map(predictions_spattemp$data, exp(est2)) + 
  ...
  theme_map() + 
  theme(legend.position = c(.85,.05))
ggsave(filename = paste0(path, "fits/1/","pos_catch_base.jpg"), plot = base_est2, dpi = 300)


cov_est1 <- plot_map(predictions_d$data, plogis(est1)) + 
  ggtitle("Covariate model: Binomial model estimate") +
  scale_fill_viridis_c(option = "G", direction = -1) +
  labs(fill = "Encounter\nprobability") +
  theme_map() + 
  theme(legend.position = c(.85,.05))
ggsave(filename = paste0(path, "fits/1/","enc_prob_cov.jpg"), plot = cov_est1)

cov_est2 <- plot_map(predictions_d$data, exp(est2)) + 
  ggtitle("Covariate model: Lognormal model estimate") +
  scale_fill_viridis_c(option = "G", direction = -1) +
  labs(fill = "Biomass\n(kg)") +
  theme_map() + 
  theme(legend.position = c(.85,.05))
ggsave(filename = paste0(path, "fits/1/","pos_catch_cov.jpg"), plot = cov_est2, dpi = 300)
...
ggsave(filename = paste0(path, "fits/1/","ff_effects_base1.jpg"), plot = base_ff1)
...
base_ff2 <- plot_map2(predictions_spattemp$data, est_non_rf2) + 
  ...
  theme_map() + 
  theme(legend.position = c(.85,.85))
ggsave(filename = paste0(path, "fits/1/","ff_effects_base2.jpg"), plot = base_ff2)

cov_ff1 <- plot_map2(predictions_d$data, est_non_rf1) + 
  scale_fill_viridis_c(option = "G", direction = -1) +
  labs(title = "Covariate model, binomial component: Fixed effects only", 
       subtitle = "Fixed effects: fyear + depth") +
  theme_map() + 
  theme(legend.position = c(.85,.85))
ggsave(filename = paste0(path, "fits/1/","ff_effects_cov1.jpg"), plot = cov_ff1)

cov_ff2 <- plot_map2(predictions_d$data, est_non_rf2) + 
  scale_fill_viridis_c(option = "G", direction = -1) +
  labs(title = "Covariate model, lognormal component: Fixed effects only", 
       subtitle = "Fixed effects: fyear + depth") +
  theme_map() + 
  theme(legend.position = c(.85,.85))
ggsave(filename = paste0(path, "fits/1/","ff_effects_cov2.jpg"), plot = cov_ff2)
...
cov_omega1 <- plot_map2(predictions_d$data, omega_s1) + 
  scale_fill_viridis_c(option = "G", direction = -1) +
...
cov_omega2 <- plot_map2(predictions_d$data, omega_s2) + 
  scale_fill_viridis_c(option = "G", direction = -1) +
...
ggsave(filename = paste0(path, "fits/1/","all_omega_base_cov.jpg"), plot = omega)
...
# covariate model
predictions_cov_om1 <- predict(mod_depth, 
                               newdata = grid_yrs,
                               model = 1, nsim = 500, 
                               sims_var = "omega_s")
...
predictions_cov_om2 <- predict(mod_depth, 
                               newdata = grid_yrs, 
                               model = 2, nsim = 500, 
                               sims_var = "omega_s")
...
omega_se <- (omega_uncertainty_base1 + omega_uncertainty_base2) / 
  ...
  plot_annotation(title = "Point-wise uncertainty in spatial predictions",
                  theme = theme(plot.title = element_text(hjust = 0.45, size = 16, face = "bold")))
ggsave(filename = paste0(path, "fits/1/","all_omega_se_base_cov.jpg"), plot = omega_se)

# SPATIOTEMPORAL EFFECTS
#base_preds <- read.csv(file = paste0(path, "fits/1/","base_model_predictions.csv"))
base_eps1 <- plot_map(predictions_spattemp$data, epsilon_st1) + 
  ...
  theme_map() +
  theme(legend.position = c(.85,.05))
ggsave(filename = paste0(path, "fits/1/","eps_effects_base1.jpg"), plot = base_eps1, dpi = 300)
...
base_eps2 <- plot_map(predictions_spattemp$data, epsilon_st2) + 
  ...
  theme_map() +
  theme(legend.position = c(.85,.05))
ggsave(filename = paste0(path, "fits/1/","eps_effects_base2.jpg"), plot = base_eps2, dpi = 300)

#cov_preds <- read.csv(file = paste0(path, "fits/1/","cov_model_predictions.csv"))
cov_eps1 <- plot_map(predictions_d$data, epsilon_st1) + 
  scale_fill_viridis_c(option = "G", direction = -1) +
  labs(title = "Covariate model, binomial component: Spatiotemporal random effects only", 
       fill = "Estimate") +
  theme_map() +
  theme(legend.position = c(.85,.05))
ggsave(filename = paste0(path, "fits/1/","eps_effects_cov1.jpg"), plot = cov_eps1, dpi = 300)

cov_eps2 <- plot_map(predictions_d$data, epsilon_st2) + 
  scale_fill_viridis_c(option = "G", direction = -1) +
  labs(title = "Covariate model, lognormal component: Spatiotemporal random effects only", 
       fill = "Estimate") +
  theme_map() +
  theme(legend.position = c(.85,.05))
ggsave(filename = paste0(path, "fits/1/","eps_effects_cov2.jpg"), plot = cov_eps2, dpi = 300)
...
eps_uncertainty_base1 <- plot_map(grid_yrs, base_eps1_se) +
  ...
  theme_map() +
  theme(legend.position = c(.85,.05))
ggsave(filename = paste0(path, "fits/1/","eps_se_base1.jpg"), plot = eps_uncertainty_base1, dpi = 300)
...
eps_uncertainty_base2 <- plot_map(grid_yrs, base_eps2_se) +
  ...
  theme_map() +
  theme(legend.position = c(.85,.05))
ggsave(filename = paste0(path, "fits/1/","eps_se_base2.jpg"), plot = eps_uncertainty_base2, dpi = 300)


# covariate model
predictions_cov_eps1 <- predict(mod_depth, 
                                newdata = grid_yrs,
                                model = 1, nsim = 500, 
                                sims_var = "epsilon_st")
...
predictions_cov_eps2 <- predict(mod_depth, 
                                newdata = grid_yrs,
                                model = 2, nsim = 500, 
                                sims_var = "epsilon_st")
...
eps_uncertainty_cov1 <- plot_map(grid_yrs, cov_eps1_se) +
  ...
  theme_map() +
  theme(legend.position = c(.85,.05))
ggsave(filename = paste0(path, "fits/1/","eps_se_cov1.jpg"), plot = eps_uncertainty_cov1)
...
eps_uncertainty_cov2 <- plot_map(grid_yrs, cov_eps2_se) +
  ...
  theme_map() +
  theme(legend.position = c(.85,.05))
ggsave(filename = paste0(path, "fits/1/","eps_se_cov2.jpg"), plot = eps_uncertainty_cov2)

# Save all predictions
#save(grid_yrs, file = paste0(path, "fits/1/", "uncertainty_mapping_data.RData"))
