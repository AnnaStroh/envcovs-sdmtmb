...
load(paste0(path, "fits/0/0_aic.RData"))
aic <- aic[order(aic$AIC), ]
min(aic$AIC) # m_dlogn6

sink(file = paste0(path, "/fits/0/", "aic.txt"))
aic
sink(file = NULL)

### Read data ------
mods_0 <- readRDS(paste0(path, "fits/0/0_model_object.rds"))
names(mods_0)

mods_0$m_dlogn6[[1]]$formula

mod_spattemp <- mods_0$m_dlogn_spattemp[[1]]
#mod_spattemp_iso <- mods_0$m_dlogn_spattemp_iso[[1]]
mod_depth_subst <- mods_0$m_dlogn6[[1]]
#mod_depth <- mods_0$m_dlogn3[[1]]

sanity(mod_spattemp)
sanity(mod_depth_subst)

sink(file = paste0(path, "/fits/0/", "base_model.txt"))
mod_spattemp
...
sink(file = paste0(path, "/fits/0/", "cov_model.txt"))
mod_depth_subst
...
mod_spattemp_iso$tmb_params$ln_kappa
mods_0$m_dlogn_spattemp_iso[[2]]
mods_0$m_dlogn_spattemp[[2]]

mods_0$m_dlogn_spattemp_iso[[3]]
mods_0$m_dlogn_spattemp[[3]]
...
### Visualise indices ------
index_0 <- ggplot(all_indices, aes(year, est, colour = model), size=2) + geom_line() +
  ...
  ggtitle("Abundance index by model approach: Whiting age 0")
ggsave(filename = paste0(path, "/fits/0/","logn_indices.jpg"), plot = index_0)
saveRDS(index_0, file = paste0(plot_path, "Age0_index_ribbon.rds"))

index_0_2 <- ggplot(all_indices, aes(year, est, colour = model, linetype = model), size=2) + 
...
ggsave(filename = paste0(path, "/fits/0/","indices2_pred.jpg"), plot = index_0_2)
saveRDS(index_0_2, file = paste0(plot_path, "/0_index2.rds"))

### Visualise index error ------
all_indices$CI_width <- with(all_indices, upr - lwr)
head(all_indices)

index_0_error <- ggplot(all_indices, aes(year, CI_width, colour = model, linetype = model), size=2) + 
  geom_line(linewidth = 2) +
  scale_color_manual(values = c("grey", "black")) +
  labs(fill = "Model", colour = "Model", linetype = "Model") +
  ...
  theme(axis.text = element_text(size = 15), 
        ...
        legend.background = element_rect(linewidth = 1, colour = "black"), 
        plot.margin = margin(5, 20, 5, 5))
ggsave(filename = paste0(path, "/fits/0/","0_index_CI_pred.jpg"), plot = index_0_error)
saveRDS(index_0_error, file = paste0(plot_path, "0_index_CI.rds"))

index_0_se <- ggplot(all_indices, aes(year, log(se), colour = model, linetype = model), size=2) + 
  geom_line(linewidth = 2) +
  scale_color_manual(values = c("grey", "black")) +
  labs(fill = "Model", colour = "Model", linetype = "Model") +
  ...
  theme(axis.text = element_text(size = 15), 
        ...
        legend.background = element_rect(linewidth = 1, colour = "black"), 
        plot.margin = margin(5, 20, 5, 5))
ggsave(filename = paste0(path, "/fits/0/","indices_se_pred.jpg"), plot = index_0_se)
saveRDS(index_0_se, file = paste0(plot_path, "/0_index_se.rds"))
...
save(all_rfs_sub, file = paste0(path, "/fits/0/", "rf_data.RData"))
...
rfs_plot2D <- ggplot(all_rfs_sub, aes(term, estimate, fill = model)) +
  ...
  theme(axis.text = element_text(size = 15), 
        ...
        legend.background = element_rect(linewidth = 1, colour = "black"), 
        plot.margin = margin(5, 20, 5, 5))
ggsave(filename = paste0(path, "/fits/0/","rf_2D.jpg"), plot = rfs_plot2D, 
       width = 2100, height = 2100, units = "px")
saveRDS(rfs_plot2D, file = paste0(plot_path, "/whg_0_rf.rds"))

### MARGINAL model effects and significance ----
### Base model
# year
...
ggsave(filename = paste0(path, "/fits/0/" ,"year_effect_base.jpg"), plot = y_p_base)
...
ggsave(filename = paste0(path, "/fits/0/", "year_effect_cov.jpg"), plot = y_p)
...
ggsave(filename = paste0(path, "/fits/0/", "depth_effect.jpg"), plot = d_p)
...
ggsave(filename = paste0(path, "/fits/0/", "substr_effect.jpg"), plot = substr_p)
...
saveRDS(mar_eff_list, paste0(path, "/fits/0/", "allmarginaleffects.rds"))
...
saveRDS(year_eff_list, paste0(path, "/fits/0/", "year_marginaleffects.rds"))

### Partial deviance explained (PDE) -----
mod_depth <- mods_0$m_dlogn3[[1]]
mod_substr <- mods_0$m_dlogn5[[1]]
...
sink(file = paste0(path, "/fits/0/", "pde_report.txt"))
...
write.csv(all_terms_base, 
          file = paste0(path, "/fits/0/", "base_factor_level_pvalues.csv"))
...
write.csv(all_terms_cov, 
          file = paste0(path, "/fits/0/", "cov_factor_level_pvalues.csv"))


########## -------------------------
### TESTING MODEL FIT AND DESIGN
########## -------------------------
...
cov_datasets <- holdout(model = mod_depth_subst, training_cutoff = 0.7)
...
cov_cv <- sdmTMB( 
  ...
  list(biomass ~ fyear + s(middepth_scaled, m = 1) + fsubstrate,
       biomass ~ fyear + s(middepth_scaled, m = 1) + fsubstrate),
  mesh = mesh_cov,
  family = delta_lognormal(),
...
saveRDS(file = paste0(path, "fits/0/", "cv_fits.rds"), crossvalfits)

sink(file = paste0(path, "fits/0/", "crossvalidation_results.txt"))
crossval_report(base_cv_preds, cov_cv_preds)
sink(file = NULL)

### Check residuals ------
#Looking at tails
# randomised quantile residuals - MCMC-MVN
...
png(paste0(path, "fits/0/", "randomised_quantile_base_bestfit.png"))
par(mfrow = c(2,2))
...
png(paste0(path, "fits/0/", "dharma_covbestfit_test_m1.png"))
sink(file = paste0(path, "fits/0/", "dharma_standardtests_covmod_m1.txt"))
DHARMa::testResiduals(s_mod_cov$m1)
sink(file = NULL)
dev.off()
png(paste0(path, "fits/0/", "dharma_covbestfit_test_m2.png"))
sink(file = paste0(path, "fits/0/", "dharma_standardtests_covmod_m2.txt"))
DHARMa::testResiduals(s_mod_cov$m2)
sink(file = NULL)
dev.off()
png(paste0(path, "fits/0/", "dharma_base_test_m1.png"))
sink(file = paste0(path, "fits/0/", "dharma_standardtests_basemod_m1.txt"))
DHARMa::testResiduals(s_mod_base$m1)
sink(file = NULL)
dev.off()
png(paste0(path, "fits/0/", "dharma_base_test_m2.png"))
sink(file = paste0(path, "fits/0/", "dharma_standardtests_basemod_m2.txt"))
DHARMa::testResiduals(s_mod_base$m2)
sink(file = NULL)
dev.off()

# Res vs preds
png(paste0(path, "fits/0/", "dharma_mods_vs_preds.png"))
par(mfrow = c(2,2))
...
png(paste0(path, "fits/0/", "dharma_bestfit_res_vs_covs.png"))
par(mfrow = c(3,2))
...
png(paste0(path, "fits/0/", "dharma_base_res_vs_years.png"))
par(mfrow = c(1,2))
...
# Zeroinflation
png(paste0(path, "fits/0/", "zeroinflation_mods.png"))
par(mfrow = c(2,2))
...
title(main = list("Base model (left), Covariate model (right)", cex = 1.4), 
      line = -3.3, outer = TRUE)
dev.off()
sink(file = paste0(path, "fits/0/", "dharma_zeroinf_basemod_m1.txt"))
DHARMa::testZeroInflation(s_mod_base$m1)
sink(file = NULL)
sink(file = paste0(path, "fits/0/", "dharma_zeroinf_basemod_m2.txt"))
DHARMa::testZeroInflation(s_mod_base$m2)
sink(file = NULL)
sink(file = paste0(path, "fits/0/", "dharma_zeroinf_covmod_m1.txt"))
DHARMa::testZeroInflation(s_mod_cov$m1)
sink(file = NULL)
sink(file = paste0(path, "fits/0/", "dharma_zeroinf_covmod_m2.txt"))
DHARMa::testZeroInflation(s_mod_cov$m2)
sink(file = NULL)

# Uniformity
png(paste0(path, "fits/0/", "uniformity_mods.png"))
par(mfrow = c(2,2))
...
title(main = list("Base model (left), Covariate model (right)", cex = 1.4), 
      line = -3.3, outer = TRUE)
dev.off()
sink(file = paste0(path, "fits/0/", "dharma_uniformity_basemod_m1.txt"))
DHARMa::testUniformity(s_mod_base$m1)
sink(file = NULL)
sink(file = paste0(path, "fits/0/", "dharma_uniformity_basemod_m2.txt"))
DHARMa::testUniformity(s_mod_base$m2)
sink(file = NULL)
sink(file = paste0(path, "fits/0/", "dharma_uniformity_covmod.txt"))
DHARMa::testUniformity(s_mod_cov$m1)
sink(file = NULL)
sink(file = paste0(path, "fits/0/", "dharma_uniformity_covmod.txt"))
DHARMa::testUniformity(s_mod_cov$m2)
...
# Dispersion
png(paste0(path, "fits/0/", "dispersion_mods.png"))
par(mfrow = c(2,2))
...
title(main = list("Base model (left), Covariate model (right)", cex = 1.4), 
      line = -3.3, outer = TRUE)
dev.off()
sink(file = paste0(path, "fits/0/", "dharma_dispersion_basemod_m1.txt"))
DHARMa::testDispersion(s_mod_base$m1)
sink(file = NULL)
sink(file = paste0(path, "fits/0/", "dharma_dispersion_basemod_m2.txt"))
DHARMa::testDispersion(s_mod_base$m2)
sink(file = NULL)
sink(file = paste0(path, "fits/0/", "dharma_dispersion_covmod.txt"))
DHARMa::testDispersion(s_mod_cov$m1)
sink(file = NULL)
sink(file = paste0(path, "fits/0/", "dharma_dispersion_covmod.txt"))
DHARMa::testDispersion(s_mod_cov$m2)
...
png(paste0(path, "fits/0/", "tempautocorr_basemod_m1.png"))
...
title(main = list("Base model", cex = 1.4), 
      line = -1, outer = TRUE)
dev.off()
png(paste0(path, "fits/0/", "tempautocorr_basemod_m2.png"))
DHARMa::testTemporalAutocorrelation(grouped_basemod2, time = unique(mod_plot_data$fyear))
title(main = list("Base model", cex = 1.4), 
      line = -1, outer = TRUE)
dev.off()
png(paste0(path, "fits/0/", "tempautocorr_covmod_m1.png"))
...
title(main = list("Covariate model", cex = 1.4), 
      line = -1, outer = TRUE)
dev.off()
png(paste0(path, "fits/0/", "tempautocorr_covmod_m2.png"))
...
sink(file = NULL)
sink(file = paste0(path, "fits/0/", "dharma_dispersion_covmod_m1.txt"))
testTemporalAutocorrelation(grouped_covmod1, time = unique(mod_depth_subst$data$fyear))
sink(file = NULL)
sink(file = paste0(path, "fits/0/", "dharma_dispersion_covmod_m2.txt"))
...
ggsave(filename = paste0(path, "fits/0/", "spatial_res_basemod_m1.png"), plot = spatial_basemod1)
spatial_basemod2 <- ggplot(mod_spattemp$data, aes(X, Y, col = resids2)) + scale_colour_gradient2() +
  geom_point() + facet_wrap(~year) + coord_fixed()
ggsave(filename = paste0(path, "fits/0/", "spatial_res_basemod_m2.png"), plot = spatial_basemod2)
spatial_covmod1 <- ggplot(mod_depth_subst$data, aes(X, Y, col = resids)) + scale_colour_gradient2() +
  geom_point() + facet_wrap(~year) + coord_fixed()
ggsave(filename = paste0(path, "fits/0/", "spatial_res_covmod_m1.png"), plot = spatial_covmod1)
spatial_covmod2 <- ggplot(mod_depth_subst$data, aes(X, Y, col = resids2)) + scale_colour_gradient2() +
  geom_point() + facet_wrap(~year) + coord_fixed()
ggsave(filename = paste0(path, "fits/0/", "spatial_res_covmod_m2.png"), plot = spatial_covmod2)
...
sink(file = paste0(path, "fits/0/","global_morans_results_base_binom.txt"))
...
morans_spattemp_m1
sink(file = NULL)
sink(file = paste0(path, "fits/0/","global_morans_results_base_logn.txt"))
cat("Sample size in each year \n")
n_spattemp_m2
cat("\n")
cat("Moran's I in each year \n")
...
ggsave(filename = paste0(path, "fits/0/", "globm_plots_base_binom.jpg"), 
...
buffered_spattemp_m2_grid <- cowplot::ggdraw() +
  cowplot::draw_plot(globmorans_spattemp_m2_grid, 
                     x = 0.05, y = 0.05, width = 0.9, height = 0.9)
ggsave(filename = paste0(path, "fits/0/", "globm_plots_base_logn.jpg"), 
...
sink(file = paste0(path, "fits/0/","global_morans_results_cov_binom.txt"))
cat("Sample size in each year \n")
n_cov_m1
cat("\n")
cat("Moran's I in each year \n")
morans_cov_m1
sink(file = NULL)
sink(file = paste0(path, "fits/0/","global_morans_results_cov_logn.txt"))
cat("Sample size in each year \n")
n_cov_m2
cat("\n")
cat("Moran's I in each year \n")
...
cowplot::save_plot(paste0(path, "fits/0/", "globm_plots_cov_binom.jpg"), 
                   globmorans_cov_m1_grid)
...
cowplot::save_plot(paste0(path, "fits/0/", "locm_plots_cov_logn.jpg"), 
                   globmorans_cov_m2_grid)
...
save(locm_spattemp_binom_df, 
     file = paste0(path, "fits/0/","local_morans_psignificance_base_binom.RData"))
locm_spattemp_binom_sig <- purrr::map(locm_spattemp_binom, ~ check_psig_locm(.x))
sink(file = paste0(path, "fits/0/","local_morans_psignificance_base_binom.txt"))
...
save(locm_spattemp_pos_df, 
     file = paste0(path, "fits/0/","local_morans_psignificance_base_pos.RData"))
locm_spattemp_pos_sig <- purrr::map(locm_spattemp_pos, ~ check_psig_locm(.x))
sink(file = paste0(path, "fits/0/","local_morans_psignificance_base_pos.txt"))
print("Sum of coordinates with p-value significance below alpha level threshold 0.01")
...
save(locm_cov_binom_df, 
     file = paste0(path, "fits/0/","local_morans_psignificance_cov_binom.RData"))
locm_cov_binom_sig <- purrr::map(locm_cov_binom, ~ check_psig_locm(.x))
sink(file = paste0(path, "fits/0/","local_morans_psignificance_cov_binom.txt"))
print("Sum of coordinates with p-value significance below alpha level threshold 0.01")
...
save(locm_cov_pos_df, 
     file = paste0(path, "fits/0/","local_morans_psignificance_cov_pos.RData"))
locm_cov_pos_sig <- purrr::map(locm_cov_pos, ~ check_psig_locm(.x))
sink(file = paste0(path, "fits/0/","local_morans_psignificance_cov_pos.txt"))
print("Sum of coordinates with p-value significance below alpha level threshold 0.01")
...
save(getisord_spattemp_binom_df, 
     file = paste0(path, "fits/0/","getis_ord_psignificance_spattemp_binom.RData"))
getisord_spattemp_binom_sig <- purrr::map(getisord_spattemp_binom, ~ check_psig_locg(.x))
sink(file = paste0(path, "fits/0/","getis_ord_psignificance_base_binom.txt"))
print("Sum of coordinates with p-value significance below alpha level threshold 0.01")
...
save(getisord_spattemp_pos_df, 
     file = paste0(path, "fits/0/","getis_ord_psignificance_spattemp_pos.RData"))
getisord_spattemp_pos_sig <- purrr::map(getisord_spattemp_pos, ~ check_psig_locg(.x))
sink(file = paste0(path, "fits/0/","getis_ord_psignificance_base_pos.txt"))
print("Sum of coordinates with p-value significance below alpha level threshold 0.01")
...
save(getisord_cov_binom_df, 
     file = paste0(path, "fits/0/","getis_ord_psignificance_cov_binom.RData"))
getisord_cov_binom_sig <- purrr::map(getisord_cov_binom, ~ check_psig_locg(.x))
sink(file = paste0(path, "fits/0/","getis_ord_psignificance_cov_binom.txt"))
print("Sum of coordinates with p-value significance below alpha level threshold 0.01")
...
save(getisord_cov_pos_df, 
     file = paste0(path, "fits/0/","getis_ord_psignificance_cov_pos.RData"))
getisord_cov_pos_sig <- purrr::map(getisord_cov_pos, ~ check_psig_locg(.x))
sink(file = paste0(path, "fits/0/","getis_ord_psignificance_cov_pos.txt"))
print("Sum of coordinates with p-value significance below alpha level threshold 0.01")
...
saveRDS(spat_autocor_res, file = paste0(path, "fits/0/", "spatialAutocorResults.rds"))
...
cowplot::save_plot(paste0(path, "fits/0/","locm_plots_base_logn.jpg"), 
...
cowplot::save_plot(paste0(path, "fits/0/", "locm_plots_cov_binom.jpg"), 
...
cowplot::save_plot(paste0(path, "fits/0/", "getis_ord_plots_base_logn.jpg"), 
...
cowplot::save_plot(paste0(path, "fits/0/", "getis_ord_plots_cov_binom.jpg"), 
...
cowplot::save_plot(paste0(path, "fits/0/", "getis_ord_plots_cov_logn.jpg"), 
...
cowplot::save_plot(paste0(path, "fits/0/","getis_ord_hotspot_plots_base_logn.jpg"), 
...
cowplot::save_plot(paste0(path, "fits/0/","getis_ord_hotspot_plots_cov_binom.jpg"), 
...
cowplot::save_plot(paste0(path, "fits/0/","getis_ord_hotspot_plots_cov_logn.jpg"), 
...
saveRDS(predictions_spattemp, file = paste0(path, "fits/0/","fullobj_base_model_predictions.rds"))
write.csv(predictions_spattemp$data, file = paste0(path, "fits/0/","base_model_predictions.csv"), row.names = FALSE)

predictions_d_s$data$density <- plogis(predictions_d_s$data$est1) * exp(predictions_d_s$data$est2)
saveRDS(predictions_d_s, file = paste0(path, "fits/0/","fullobj_cov_model_predictions.rds"))
write.csv(predictions_d_s$data, file = paste0(path, "fits/0/","cov_model_predictions.csv"), row.names = FALSE)

base_preds <- read.csv(file = paste0(path, "fits/0/","base_model_predictions.csv"))
cov_preds <- read.csv(file = paste0(path, "fits/0/","cov_model_predictions.csv"))

# DENSITY
base_density <- plot_map(base_preds, density) + 
  ggtitle("Base model: Density") +
  scale_fill_viridis_c(option = "G", direction = -1) +
  theme_map() + 
  theme(legend.position = c(.85,.05))
ggsave(filename = paste0(path, "fits/0/","density_base.jpg"), plot = base_density, dpi = 300)

cov_density <- plot_map(cov_preds, density) + 
  ggtitle("Covariate model: Density") +
  scale_fill_viridis_c(option = "G", direction = -1) +
  theme_map() + 
  theme(legend.position = c(.85,.05))
ggsave(filename = paste0(path, "fits/0/","density_cov.jpg"), plot = cov_density, dpi = 300)
...
ggsave(filename = paste0(path, "fits/0/","est_se_base1.jpg"), plot = est_uncertainty_base1)
...
est_uncertainty_base2 <- plot_map(grid_yrs, base_est2_se) +
  ...
  theme(plot.title = element_text(hjust = 0.5, size = 13, face = "bold"),
        legend.position = c(.85,.05))
ggsave(filename = paste0(path, "fits/0/","est_se_base2.jpg"), plot = est_uncertainty_base2)
...
est_uncertainty_cov1 <- plot_map(grid_yrs, cov_est1_se) +
  ...
  theme(plot.title = element_text(hjust = 0.5, size = 13, face = "bold"),
        legend.position = c(.85,.05))
ggsave(filename = paste0(path, "fits/0/","est_se_cov1.jpg"), plot = est_uncertainty_cov1)
...
est_uncertainty_cov2 <- plot_map(grid_yrs, cov_est2_se) +
  ...
  theme(plot.title = element_text(hjust = 0.5, size = 13, face = "bold"),
        legend.position = c(.85,.05))
ggsave(filename = paste0(path, "fits/0/","est_se_cov2.jpg"), plot = est_uncertainty_cov2)

# FIXED AN RANDOM EFFECTS
base_est1 <- plot_map(base_preds, plogis(est1)) + 
  ggtitle("Base model: Binomial model estimate") +
  scale_fill_viridis_c(option = "G", direction = -1) +
  labs(fill = "Encounter\nprobability") +
  theme_map() + 
  theme(legend.position = c(.85,.05))
ggsave(filename = paste0(path, "fits/0/","enc_prob_base.jpg"), plot = base_est1, dpi = 300)

base_est2 <- plot_map(base_preds, exp(est2)) + 
  ggtitle("Base model: Lognormal model estimate") +
  scale_fill_viridis_c(option = "G", direction = -1) +
  labs(fill = "Biomass\n(kg)") +
  theme_map() + 
  theme(legend.position = c(.85,.05))
ggsave(filename = paste0(path, "fits/0/","pos_catch_base.jpg"), plot = base_est2, dpi = 300)


cov_est1 <- plot_map(cov_preds, plogis(est1)) + 
  ggtitle("Covariate model: Binomial model estimate") +
  scale_fill_viridis_c(option = "G", direction = -1) +
  labs(fill = "Encounter\nprobability") +
  theme_map() +
  theme(legend.position = c(.85,.05))
ggsave(filename = paste0(path, "fits/0/","enc_prob_cov.jpg"), plot = cov_est1)

cov_est2 <- plot_map(cov_preds, exp(est2)) + 
  ggtitle("Covariate model: Lognormal model estimate") +
  scale_fill_viridis_c(option = "G", direction = -1) +
  labs(fill = "Biomass\n(kg)") +
  theme_map() +
  theme(legend.position = c(.85,.05))
ggsave(filename = paste0(path, "fits/0/","pos_catch_cov.jpg"), plot = cov_est2, dpi = 300)


# FIXED EFFECTS
base_ff1 <- plot_map2(base_preds, est_non_rf1) + 
  scale_fill_viridis_c(option = "G", direction = -1) +
  labs(title = "Base model, binomial component: Fixed effects only", 
       subtitle = "Fixed effects: fyear") +
  theme_map() + 
  theme(legend.position = c(.85,.85))
ggsave(filename = paste0(path, "fits/0/","ff_effects_base1.jpg"), plot = base_ff1)

base_ff2 <- plot_map2(base_preds, est_non_rf2) + 
  scale_fill_viridis_c(option = "G", direction = -1) +
  labs(title = "Base model, lognormal component: Fixed effects only", 
       subtitle = "Fixed effects: fyear") +
  theme_map() + 
  theme(legend.position = c(.85,.85))
ggsave(filename = paste0(path, "fits/0/","ff_effects_base2.jpg"), plot = base_ff2)

cov_ff1 <- plot_map2(cov_preds, est_non_rf1) + 
  scale_fill_viridis_c(option = "G", direction = -1) +
  labs(title = "Covariate model, binomial component: Fixed effects only", 
       subtitle = "Fixed effects: fyear + depth + substrate") +
  theme_map() + 
  theme(legend.position = c(.85,.85))
ggsave(filename = paste0(path, "fits/0/","ff_effects_cov1.jpg"), plot = cov_ff1)

cov_ff2 <- plot_map2(cov_preds, est_non_rf2) + 
  scale_fill_viridis_c(option = "G", direction = -1) +
  labs(title = "Covariate model, lognormal component: Fixed effects only", 
       subtitle = "Fixed effects: fyear + depth + substrate") +
  theme_map() + 
  theme(legend.position = c(.85,.85))
ggsave(filename = paste0(path, "fits/0/","ff_effects_cov2.jpg"), plot = cov_ff2)

# SPATIAL EFFECTS
base_omega1 <- plot_map2(base_preds, omega_s1) + 
  scale_fill_viridis_c(option = "G", direction = -1) +
...
base_omega2 <- plot_map2(base_preds, omega_s2) + 
  scale_fill_viridis_c(option = "G", direction = -1) +
...
cov_omega1 <- plot_map2(cov_preds, omega_s1) + 
  scale_fill_viridis_c(option = "G", direction = -1) +
...
cov_omega2 <- plot_map2(cov_preds, omega_s2) + 
  scale_fill_viridis_c(option = "G", direction = -1) +
...
ggsave(filename = paste0(path, "fits/0/","all_omega_base_cov.jpg"), plot = omega)
...
omega_uncertainty_base1 <- plot_map2(grid_yrs, base_om1_se) +
  ...
  theme(plot.title = element_text(hjust = 0.5, size = 13, face = "bold"),
        legend.position = c(.85,.65))
#ggsave(filename = paste0(path, "fits/0/","omega_sd_base1.png"), plot = omega_uncertainty_base1)
...
omega_uncertainty_base2 <- plot_map2(grid_yrs, base_om2_se) +
  ...
  theme(plot.title = element_text(hjust = 0.5, size = 13, face = "bold"),
        legend.position = c(.85,.65))
#ggsave(filename = paste0(path, "fits/0/","omega_sd_base.png"), plot = omega_uncertainty_base)
...
omega_uncertainty_cov2 <- plot_map2(grid_yrs, cov_om2_se) +
  ...
  theme(plot.title = element_text(hjust = 0.5, size = 13, face = "bold"),
        legend.position = c(.85,.65))
#ggsave(filename = paste0(path, "fits/0/","omega_sd_cov.png"), plot = omega_uncertainty_cov)
...
omega_se <- (omega_uncertainty_base1 + omega_uncertainty_base2) / 
  ...
  plot_annotation(title = "Point-wise uncertainty in spatial predictions",
                  theme = theme(plot.title = element_text(hjust = 0.45, size = 16, face = "bold")))
ggsave(filename = paste0(path, "fits/0/","all_omega_se_base_cov.jpg"), plot = omega_se)
...
base_eps1 <- plot_map(base_preds, epsilon_st1) + 
  ...
  theme_map() +
  theme(legend.position = c(.85,.05))
ggsave(filename = paste0(path, "fits/0/","eps_effects_base1.jpg"), plot = base_eps1, dpi = 300)
...
base_eps2 <- plot_map(base_preds, epsilon_st2) + 
  ...
  theme_map() +
  theme(legend.position = c(.85,.05))
ggsave(filename = paste0(path, "fits/0/","eps_effects_base2.jpg"), plot = base_eps2, dpi = 300)
...
cov_eps1 <- plot_map(cov_preds, epsilon_st1) + 
  ...
  theme_map() +
  theme(legend.position = c(.85,.05))
ggsave(filename = paste0(path, "fits/0/","eps_effects_cov1.jpg"), plot = cov_eps1, dpi = 300)
...
cov_eps2 <- plot_map(cov_preds, epsilon_st2) + 
  ...
  theme_map() +
  theme(legend.position = c(.85,.05))
ggsave(filename = paste0(path, "fits/0/","eps_effects_cov2.jpg"), plot = cov_eps2, dpi = 300)
...
eps_uncertainty_base1 <- plot_map(grid_yrs, base_eps1_se) +
  ...
  theme_map() +
  theme(legend.position = c(.85,.05))
ggsave(filename = paste0(path, "fits/0/","eps_se_base1.jpg"), plot = eps_uncertainty_base1, dpi = 300)
...
eps_uncertainty_base2 <- plot_map(grid_yrs, base_eps2_se) +
  ...
  theme_map() +
  theme(legend.position = c(.85,.05))
ggsave(filename = paste0(path, "fits/0/","eps_se_base2.jpg"), plot = eps_uncertainty_base2, dpi = 300)
...
eps_uncertainty_cov1 <- plot_map(grid_yrs, cov_eps1_se) +
  ...
  theme_map() +
  theme(legend.position = c(.85,.05))
ggsave(filename = paste0(path, "fits/0/","eps_se_cov1.jpg"), plot = eps_uncertainty_cov1)
...
eps_uncertainty_cov2 <- plot_map(grid_yrs, cov_eps2_se) +
  ...
  theme_map() +
  theme(legend.position = c(.85,.05))
ggsave(filename = paste0(path, "fits/0/","eps_se_cov2.jpg"), plot = eps_uncertainty_cov2)

# Save all predictions
#save(grid_yrs, file = paste0(path, "fits/0/", "uncertainty_mapping_data.RData"))
load(paste0(path, "fits/0/", "uncertainty_mapping_data.RData"))
