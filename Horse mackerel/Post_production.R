...
abundance <- read.csv("caa_2025.csv")
abundance <- abundance[abundance$year >= 2003, 
                       c(1, 3:7)] # also remove age 0 index
long <- tidyr::pivot_longer(abundance, names_to = "age", cols = 2:6)
...
png(filename = paste0(path, "fits/", "intconsistency_base0.png"))
...
png(filename = paste0(path, "fits/", "intconsistency_cov0.png"))
...
saveRDS(consistency_dat, file = paste0(path, "fits/", "consistency_data.rds"))
...
#set.seed(63) # required for random sampling
cov_datasets <- holdout(model = mod_mldxsst, training_cutoff = 0.7)
...
base_cv <- sdmTMB(
  ...
  mesh = mesh_base,
  family = delta_lognormal(),
  offset = log(base_datasets$training$sweptareakm2),
  spatial = list("on","on"),
  time = "year", 
...
cov_cv <- sdmTMB( # does not converge
  ...
  mesh = mesh_cov,
  family = delta_lognormal(),
  offset = log(cov_datasets$training$sweptareakm2),
  spatial = list("on","on"),
  time = "year", 
  spatiotemporal = list("IID", "IID"),
  anisotropy = TRUE,
...
saveRDS(file = paste0(path, "fits/", "cv_fits.rds"), crossvalfits)

sink(file = paste0(path, "fits/", "crossvalidation_results.txt"))
